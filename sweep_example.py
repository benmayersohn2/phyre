"""
sweep_example.py: Example of a sweep of runs that comes with PHYRE model.

We vary the parameter zoo_mort_rate from a low to high value, to examine effect on phytoplankton
...
bio = {'zoo_low_pref': zoo_low_pref, 'zoo_high_pref': zoo_high_pref, 'phy_small_indices': phy_small_indices,
       'zoo_slop_feed': zoo_slop_feed, 'include_zoo': include_zoo,
       'res_phy_makeup_ratio': res_phy_makeup_ratio, 'phy_large_indices': phy_large_indices,
       'res_export_frac': res_export_frac,
       ...
       'num_phy': num_phy, 'turnover_rate': turnover_rate
       }
...
num_clusters = 2  # split output between two clusters

zoo_mort_rate_range = np.linspace(0.01, 0.5, 4).tolist()

sweep_params = [['zoo_mort_rate', None, list(zoo_mort_rate_range)]]
